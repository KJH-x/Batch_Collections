...
def on_tray_doubleclick(process: subprocess.Popen):
    """
    Callback function that is called when the tray icon is double-clicked.
    """
    toggle_window_visibility(process)
...
def create_tray_icon(process: subprocess.Popen):
    ...
    tray_instance = pystray.Icon('双击显示/隐藏程序框，右键功能菜单', icon_image)
    tray_instance.menu = pystray.Menu(
        pystray.MenuItem('显示/隐藏', lambda: on_tray_click(process)),
        pystray.MenuItem('退出', lambda: terminate_program(process,tray_instance))
    )
